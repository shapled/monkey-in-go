...
func NewVM(bytecode *compiler.Bytecode) *VM {
	...
	mainClousure := &object.Closure{Fn: mainFn}
	mainFrame := NewFrame(mainClousure, 0)
...
func (vm *VM) Run() error {
	for vm.currentFrame().ip+1 < len(vm.currentFrame().Instructions()) {
		...
		case code.OpCall:
			numArgs := code.ReadUint8(ins[vm.currentFrame().ip+1:])
		...
		case code.OpGetBuiltin:
			builtinIndex := code.ReadUint8(ins[vm.currentFrame().ip+1:])
			vm.currentFrame().ip += 1
		...
		case code.OpGetFree:
			freeIndex := code.ReadUint8(ins[vm.currentFrame().ip+1:])
			vm.currentFrame().ip += 1
